...
namespace Saro.Diagnostics
{
    ...
    public static class GLDebug
    {
                                       ...
                                       float drawDuration = 0, EGLDebug preview = EGLDebug.Editor)
        {
            ...
            if (drawGame)
            {
                GLDebugMono.DrawLine(topMinY1, botMinY1, color, drawDuration);
                GLDebugMono.DrawLine(topMaxY1, botMaxY1, color, drawDuration);
                GLDebugMono.DrawLine(topMinY1, topMaxY1, color, drawDuration);
                GLDebugMono.DrawLine(botMinY1, botMaxY1, color, drawDuration);
            }
        }
                                    ...
                                    EGLDebug preview = EGLDebug.Editor)
        {
            ...
            if (drawGame)
            {
                ...
                if (drawBase)
                {
                    GLDebugMono.DrawLine(topMinX1, botMinX1, color, drawDuration);
                    GLDebugMono.DrawLine(topMaxX1, botMaxX1, color, drawDuration);
                    GLDebugMono.DrawLine(topMinY1, botMinY1, color, drawDuration);
                    GLDebugMono.DrawLine(topMaxY1, botMaxY1, color, drawDuration);

                    GLDebugMono.DrawLine(topMinX1, topMaxX1, color, drawDuration);
                    GLDebugMono.DrawLine(topMinX1, topMinY1, color, drawDuration);
                    GLDebugMono.DrawLine(topMinY1, topMaxY1, color, drawDuration);
                    GLDebugMono.DrawLine(topMaxY1, topMaxX1, color, drawDuration);

                    GLDebugMono.DrawLine(botMinX1, botMaxX1, color, drawDuration);
                    GLDebugMono.DrawLine(botMinX1, botMinY1, color, drawDuration);
                    GLDebugMono.DrawLine(botMinY1, botMaxY1, color, drawDuration);
                    GLDebugMono.DrawLine(botMaxY1, botMaxX1, color, drawDuration);
                }
                #endregion

                #region Connection between boxes
                GLDebugMono.DrawLine(topMinX0, topMinX1, color, drawDuration);
                GLDebugMono.DrawLine(topMaxX0, topMaxX1, color, drawDuration);
                GLDebugMono.DrawLine(topMinY0, topMinY1, color, drawDuration);
                GLDebugMono.DrawLine(topMaxY0, topMaxY1, color, drawDuration);

                GLDebugMono.DrawLine(botMinX0, botMinX1, color, drawDuration);
                GLDebugMono.DrawLine(botMinX0, botMinX1, color, drawDuration);
                GLDebugMono.DrawLine(botMinY0, botMinY1, color, drawDuration);
                GLDebugMono.DrawLine(botMaxY0, botMaxY1, color, drawDuration);
                #endregion

                #region End box
                color = endColor;

                GLDebugMono.DrawLine(topMinX0, botMinX0, color, drawDuration);
                GLDebugMono.DrawLine(topMaxX0, botMaxX0, color, drawDuration);
                GLDebugMono.DrawLine(topMinY0, botMinY0, color, drawDuration);
                GLDebugMono.DrawLine(topMaxY0, botMaxY0, color, drawDuration);

                GLDebugMono.DrawLine(topMinX0, topMaxX0, color, drawDuration);
                GLDebugMono.DrawLine(topMinX0, topMinY0, color, drawDuration);
                GLDebugMono.DrawLine(topMinY0, topMaxY0, color, drawDuration);
                GLDebugMono.DrawLine(topMaxY0, topMaxX0, color, drawDuration);

                GLDebugMono.DrawLine(botMinX0, botMaxX0, color, drawDuration);
                GLDebugMono.DrawLine(botMinX0, botMinY0, color, drawDuration);
                GLDebugMono.DrawLine(botMinY0, botMaxY0, color, drawDuration);
                GLDebugMono.DrawLine(botMaxY0, botMaxX0, color, drawDuration);
                #endregion
            }
            #endregion
        }
                                                ...
                                                EGLDebug preview = EGLDebug.Editor)
        {
            ...
            {
                //Side lines
                GLDebugMono.DrawLine(baseSphere + right, endSphere + right, color, drawDuration);
                GLDebugMono.DrawLine(baseSphere - right, endSphere - right, color, drawDuration);
                ...
                {
                    //Start endcap
                    GLDebugMono.DrawLine(Vector3.Slerp(right, -up, i / 25.0f) + baseSphere, Vector3.Slerp(right, -up, (i - 1) / 25.0f) + baseSphere, colorizeBase ? color : Color.red, drawDuration);
                    GLDebugMono.DrawLine(Vector3.Slerp(-right, -up, i / 25.0f) + baseSphere, Vector3.Slerp(-right, -up, (i - 1) / 25.0f) + baseSphere, colorizeBase ? color : Color.red, drawDuration);

                    //End endcap
                    GLDebugMono.DrawLine(Vector3.Slerp(right, up, i / 25.0f) + endSphere, Vector3.Slerp(right, up, (i - 1) / 25.0f) + endSphere, color, drawDuration);
                    GLDebugMono.DrawLine(Vector3.Slerp(-right, up, i / 25.0f) + endSphere, Vector3.Slerp(-right, up, (i - 1) / 25.0f) + endSphere, color, drawDuration);
                }
            }
        }
                                        ...
                                        EGLDebug preview = EGLDebug.Editor)
        {
            ...
            if (drawGame)
            {
                ...
                GLDebugMono.DrawLine(baseSphere + forward, endSphere + forward, color, drawDuration);
                GLDebugMono.DrawLine(baseSphere - forward, endSphere - forward, color, drawDuration);
                ...
                for (int i = 1; i < 26; i++)
                {
                    ...
                    GLDebugMono.DrawLine(Vector3.Slerp(right, up, i / 25.0f) + endSphere, Vector3.Slerp(right, up, (i - 1) / 25.0f) + endSphere, color, drawDuration);
                    GLDebugMono.DrawLine(Vector3.Slerp(-right, up, i / 25.0f) + endSphere, Vector3.Slerp(-right, up, (i - 1) / 25.0f) + endSphere, color, drawDuration);
                    GLDebugMono.DrawLine(Vector3.Slerp(forward, up, i / 25.0f) + endSphere, Vector3.Slerp(forward, up, (i - 1) / 25.0f) + endSphere, color, drawDuration);
                    GLDebugMono.DrawLine(Vector3.Slerp(-forward, up, i / 25.0f) + endSphere, Vector3.Slerp(-forward, up, (i - 1) / 25.0f) + endSphere, color, drawDuration);

                    //Start endcap
                    GLDebugMono.DrawLine(Vector3.Slerp(right, -up, i / 25.0f) + baseSphere, Vector3.Slerp(right, -up, (i - 1) / 25.0f) + baseSphere, colorizeBase ? color : Color.red, drawDuration);
                    GLDebugMono.DrawLine(Vector3.Slerp(-right, -up, i / 25.0f) + baseSphere, Vector3.Slerp(-right, -up, (i - 1) / 25.0f) + baseSphere, colorizeBase ? color : Color.red, drawDuration);
                    GLDebugMono.DrawLine(Vector3.Slerp(forward, -up, i / 25.0f) + baseSphere, Vector3.Slerp(forward, -up, (i - 1) / 25.0f) + baseSphere, colorizeBase ? color : Color.red, drawDuration);
                    GLDebugMono.DrawLine(Vector3.Slerp(-forward, -up, i / 25.0f) + baseSphere, Vector3.Slerp(-forward, -up, (i - 1) / 25.0f) + baseSphere, colorizeBase ? color : Color.red, drawDuration);
                }
            }
        }
                                       ...
                                       float drawDuration = 0, EGLDebug preview = EGLDebug.Editor)
        {
            ...
            for (var i = 0; i < 91; i++)
            {
                ...
                if (drawGame)
                    GLDebugMono.DrawLine(lastPoint, nextPoint, color, drawDuration);
                                      ...
                                      EGLDebug preview = EGLDebug.Editor)
        {
            ...
            if (drawGame)
            {
                GLDebugMono.DrawLine(p1, p2, color, drawDuration);
            }
        }
                                      ...
                                      EGLDebug preview = EGLDebug.Editor)
        {
            ...
            if (drawGame)
            {
                GLDebugMono.DrawRay(position + (Vector3.up * (scale * 0.5f)), -Vector3.up * scale, color, drawDuration);
                GLDebugMono.DrawRay(position + (Vector3.right * (scale * 0.5f)), -Vector3.right * scale, color, drawDuration);
                GLDebugMono.DrawRay(position + (Vector3.forward * (scale * 0.5f)), -Vector3.forward * scale, color, drawDuration);
            }
        }
                                           ...
                                           EGLDebug preview = EGLDebug.Editor)
        {
            ...
            for (int i = 1; i < 37; i++)
            {
                ...
                if (drawGame)
                {
                    GLDebugMono.DrawLine(x, new_x, color, drawDuration);
                    GLDebugMono.DrawLine(y, new_y, color, drawDuration);
                    GLDebugMono.DrawLine(z, new_z, color, drawDuration);
                }
                                          ...
                                          float drawDuration = 0, EGLDebug preview = EGLDebug.Editor)
        {
            ...
            {
                //Forward
                GLDebugMono.DrawRay(position, forwardDir, color, drawDuration);

                //Left Down
                GLDebugMono.DrawLine(position, leftDown, color, drawDuration);
                //Left Up
                GLDebugMono.DrawLine(position, leftUp, color, drawDuration);
                //Right Down
                GLDebugMono.DrawLine(position, rightDown, color, drawDuration);
                //Right Up
                GLDebugMono.DrawLine(position, rightUp, color, drawDuration);

                //Left
                GLDebugMono.DrawLine(position, left, color, drawDuration);
                //Right
                GLDebugMono.DrawLine(position, right, color, drawDuration);
                //Down
                GLDebugMono.DrawLine(position, down, color, drawDuration);
                //Up
                GLDebugMono.DrawLine(position, up, color, drawDuration);
            }
            #endregion
            ...
            for (int i = 0; i < 7; i++)
            {
                ...
                if (drawGame)
                {
                    GLDebugMono.DrawLine(lastLdPosition, nextLdPosition, color, drawDuration);
                    GLDebugMono.DrawLine(lastLuPosition, nextLuPosition, color, drawDuration);
                    GLDebugMono.DrawLine(lastRdPosition, nextRdPosition, color, drawDuration);
                    GLDebugMono.DrawLine(lastRuPosition, nextRuPosition, color, drawDuration);

                    GLDebugMono.DrawLine(lastDPosition, nextDPosition, color, drawDuration);
                    GLDebugMono.DrawLine(lastUPosition, nextUPosition, color, drawDuration);
                    GLDebugMono.DrawLine(lastRPosition, nextRPosition, color, drawDuration);
                    GLDebugMono.DrawLine(lastLPosition, nextLPosition, color, drawDuration);
                }
